...
export const tutorController = {
  ...
  payouts: async (req: Request, res: Response) => {
    try {
      console.log("course payouts ------------------------------", req.query);
  ...
  getPresignedUrl: async (req: Request, res: Response) => {
    try {
      ...
      if (!s3Key) {
        return res
          .status(400)
          .json({ error: "s3Key query parameter is required" });
      }
  ...
  cardsData: async (req: Request, res: Response) => {
    try {
      ...
      const resultEarnings: any = await orderRabbitMqClient.produce(
        data,
        operationEarnings
      );
      // console.log("first reply",resultEarnings)
      const resultCardsData: any = await tutorRabbitMqClient.produce(
        data,
        operationCardsData
      );
  ...
  graphData: async (req: Request, res: Response) => {
    try {
      ...
      // Call both RabbitMQ clients to retrieve data
      const resultBar: any = await orderRabbitMqClient.produce(
        data,
        operation1
      );
      console.log("first reply", resultBar);
      const resultPie: any = await tutorRabbitMqClient.produce(
        data,
        operation2
      );
      console.log("sedon reply", resultPie);
      const PieFetchCourse: any = await courseRabbitMqClient.produce(
        resultPie,
        operation3
      );
  ...
  getUserCourses: async (req: Request, res: Response) => {
    try {
      ...
      const result3 = await chatRabbitMqClient.produce(
        result2.courses,
        operation3
      );
  ...
  addInformation: async (req: Request, res: Response) => {
    try {
      console.log("Entered to the add infor tutor", req.body);

      const data = req.body;

      const operation = "tutor-add-Information";

      const result: any = await tutorRabbitMqClient.produce(data, operation);

      console.log(result, " ---------- add Information ");
  ...
  profileDetails: async (req: Request, res: Response) => {
    try {
      console.log("Entered to the profile info tutor", req.params);

      const data = req.params;
      const operation = "fetch-tutor-data";

      // Fetch tutor data from RabbitMQ or database
      const result: any = await tutorRabbitMqClient.produce(data, operation);
      ...
      // Generate S3 Signed URLs for profile_picture and cv
      if (result.profile_picture) {
        result.profile_key = result.profile_picture;
        result.profile_picture = await getS3SignedUrl(result.profile_picture);
      }

      if (result.cv) {
        result.cv_key = result.cv;
        result.cv = await getS3SignedUrl(result.cv);
      }

      console.log("Qualifications before processing:", result.qualifications);

      // Generate S3 Signed URLs for qualifications certificates
      if (result.qualifications && Array.isArray(result.qualifications)) {
        for (const qualification of result.qualifications) {
          if (qualification.certificate) {
            console.log("CV Key:", result.cv);
            console.log("Certificate Key:", qualification.certificate);

            qualification.certificate_key = qualification.certificate;
            qualification.certificate = await getS3SignedUrl(
